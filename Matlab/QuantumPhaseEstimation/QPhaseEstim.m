...
%     eValue = exp(2*pi*1i*res);
    ...
    %%% output
    fprintf('The state that produces the eigenvalue of the m = %g qubit matrix', m);
    U
    fprintf('for the eigenvector'); 
    u
    fprintf('with precision %g', precision);  
    fprintf(' and a failure probability of at most %g%% is: ', epsilon*100);
    state
    fprintf('i.e. state %g,', maxState);
    fprintf(' with a probability %g', max);
